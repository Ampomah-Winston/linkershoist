...
//indicate whether our application is in production or not
...
app.post('/users/login', async (req,res)=>{
    try {
       ...
       const {rows} = await pool.query(`select * from users where fname='${username}' or lname='${username}'  or email='${username}' and password='${password}'`);
       res.json(rows);
       console.log(rows)
    } catch (err) {
        console.log(err)
...
app.get('/users/search/:value', async (req,res)=>{
    try {
       ...
       const {rows} = await pool.query(`select * from users where lname ilike '${value}%' or email ilike '${value}%' `);
       res.json(rows);      
    } catch (err) {
        console.log(err)
    }
...
app.post('/users/register', async (req,res)=>{
    try {
        ...
        const newUser = await pool.query(`INSERT INTO users (id,fname,lname,email,password) 
